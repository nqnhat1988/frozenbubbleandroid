...
//---------------------------------------------------------------
{
    ...
    switch (command) {
        ...
        case 0x0F:    // Set Tempo / Special
            // F.00 = Pattern Break
            if (!param) command = CMD_PATTERNBREAK;
            else
            ...
            for (UINT i = 0; i < m_nChannels; i++) {
                ...
                if (trktagofs) {
                    while (trktagofs < dwMemLength - 8) {
                        ...
                        switch (ntag) {
                            case MMDTAG_TRK_NAMELEN:
                                trknamelen = tagdata;
                                break;
                            case MMDTAG_TRK_NAME:
                                trknameofs = tagdata;
                                break;
                        }
                        trktagofs += 8;
