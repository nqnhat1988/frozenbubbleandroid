    ...
    switch (eff) {
        ...
        case 0x10:
            if ((param & 0xF0) != 0xE0) {
                ...
                if ((param & 0xF0) == 0xF0) param = ((param << 4) | 0x0F); else param >>= 2;
            }
            break;
        case 0x20:
            if ((param & 0xF0) != 0xE0) {
                command = CMD_VOLUMESLIDE;
                if ((param & 0xF0) != 0xF0) param >>= 2;
            }
            break;
        case 0x30:
            command = CMD_RETRIG;
...
//---------------------------------------------------------------
{
        ...
        switch (block) {
            ...
            case 0x4153:
                ...
                for (i = 1; i <= m_nSamples; i++)
                    if ((Ins[i].nLength) && (!Ins[i].pSample) && (smpinfo[i] != 3) && (dwPos < dwMemLength)) {
    ...
    if ((dwTrackPos) && (npatterns) && (m_nChannels) && (ntracks)) {
        for (UINT ipat = 0; ipat < npatterns; ipat++) {
            ...
            for (UINT chn = 0; chn < m_nChannels; chn++)
                if ((patterntracks[ipat * 32 + chn]) && (patterntracks[ipat * 32 + chn] <= ntracks)) {
    ...
    // Set up envelopes
    for (UINT iIns = 1; iIns <= m_nInstruments; iIns++)
        if (Headers[iIns]) {
